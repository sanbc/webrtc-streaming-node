...
var parser = {
    ...
    JSON2packet: function (nodes, packet) {
        for(var i = 0; i < nodes.length; i++)
        {
            ...
            if(node.value){
            	if(typeof node.value != 'string') {node.value=node.value.toString();}
                packet.t(node.value); 
                }
            if(node.children)
...
function ChatRoom(connection, jid, password, XMPP, options) {
    ...
    console.log("Joined MUC as " , this.myroomjid);
    this.members = {};
    this.presMap = {};
    this.presHandlers = {};
    this.joined = false;
...
ChatRoom.prototype.initPresenceMap = function () {
    ...
    this.presMap["nodes"].push( {
        ...
        "value": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/46.0.2490.86 Safari/537.36",
...
ChatRoom.prototype.sendPresence = function (tokenPassword) {
	...
	console.log("sendPresence:",this.presMap.nodes, pres);
...
ChatRoom.prototype.addStream = function (stream, callback) {
    if(this.session) {
        // FIXME: will block switchInProgress on true value in case of exception
        console.log("ChatRoom.prototype.addStream....");
